...
#    u_n_n.x.array[inside] = params["T_source"]
    ...
    F = (
        u * v * ufl.dx
        + alpha * dt *dt * ufl.dot(ufl.grad(u), ufl.grad(v)) * ufl.dx
...
def run_transient_simulation(domain, V, a, L, u_n, u_n_n, bcs, params):
    ...
    output_path = "../../output/wave_solution_int.xdmf"
    ...
    for i in progress:
        ...
        inside_generator = np.logical_or(circle1, circle2)

        u_n.x.array[inside_generator] = np.cos(w * t) * params["T_source"]
        u_n_n.x.array[inside_generator] = np.cos(w * t) * params["T_source"]
...
#def wave_generator(domain, V, params, w, t):
#    """garmonic wave generator"""
#
#
##    # 1. Outer boundary condition (fixed at T_boundary)
##    # Since all exterior facets are at the same temperature, we can do this simply.
...
#        circle1 = (x[0]**2 + (x[1] - 0.5)**2) < 0.001
#        circle2 = (x[0]**2 + (x[1] + 0.5)**2) < 0.001
#        return np.logical_or(circle1, circle2)
#
#    source_dofs = fem.locate_dofs_geometrical(V, source_region)
#    bc_source = fem.dirichletbc(default_scalar_type(params[np.cos(w * t)*"T_source"]), source_dofs, V)
#
##    u_n.x.array[inside] = params["T_source"]
##    u_n_n.x.array[inside] = params["T_source"]
#
