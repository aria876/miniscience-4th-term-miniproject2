...
#        gmsh.model.addPhysicalGroup(2, [fluid_domain[0][1]], 5)
        ...
        gmsh.option.setNumber("Mesh.CharacteristicLengthMin", 0.005) #0.02)
        gmsh.option.setNumber("Mesh.CharacteristicLengthMax", 0.02) #0.1)
...
#Препятствие
    D1 = (abs(coords[:, 0]) < 0.2)
    D2 = (abs(coords[:, 1]-0.8) < 0.1)
    poisson_obstacle = np.logical_and(D1, D2)

#Генератор гармонических волн
    T_generator = 8 #period
...
# препятствие
        u_n.x.array[poisson_obstacle] = 0
        u_n_n.x.array[poisson_obstacle] = 0
